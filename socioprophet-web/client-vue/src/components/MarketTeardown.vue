...
// Reusable market-teardown surface. Each market supplies DATA; this renders it.
// Extracted from the market-2 page so markets 3-8 do not each need a bespoke page.
import { computed } from 'vue';
import { RouterLink } from 'vue-router';
import SurfaceHeader from './SurfaceHeader.vue';
import BoundaryNotice from './BoundaryNotice.vue';
import { serveLabel, type MarketTeardownData } from '../features/competitive-intelligence/marketTeardown';

const props = defineProps<{ data: MarketTeardownData }>();
const d = computed(() => props.data).value;

const byDurability = computed(() => [...d.specimens].sort((a, b) => b.durability - a.durability));
...
<template>
  <section class="ep" aria-labelledby="ep-title">
    <RouterLink class="ep-back" :to="d.backTo ?? '/professional-intelligence/competitive/markets'">← Market Portfolio</RouterLink>

    <SurfaceHeader :title="d.headline" :eyebrow="`Competitive Intelligence · ${d.marketLabel}`">
      <template #badge>
        <span class="ep-badge">{{ d.specimens.length }} specimens</span>
        <span class="ep-badge ep-badge--fixture">fixture</span>
      </template>
    </SurfaceHeader>

    <p id="ep-title" class="ep-lede">{{ d.lede }}</p>

    <BoundaryNotice
      label="fixture · research digest"
      tone="muted"
      :message="d.sourcing"
      aria-label="Market teardown boundary"
    />

    <p class="ep-thesis"><span class="ep-thesis-k">Thesis</span>{{ d.thesis }}</p>
    ...
    <section class="ep-block" aria-label="This market's machine">
      <div class="ep-h">
        <h2>{{ d.machineTitle }}</h2>
        <p>{{ d.machineNote }}</p>
      </div>
      <div class="ep-steps">
        <article v-for="step in d.machine" :key="step.index" class="ep-step">
        ...
        <table class="ep-table">
          ...
          <tbody>
            <tr v-for="sp in byDurability" :key="sp.name">
              ...
              <td class="ep-moat">
                <span class="ep-arch">{{ sp.moatArchetype }}</span>
                {{ sp.moat }}
              </td>
              <td class="ep-pricing">
    ...
    <section class="ep-block" aria-label="Unserved gaps">
      ...
      <div class="ep-gaps">
        <article v-for="g in d.unserved" :key="g.gap" class="ep-gap" :class="`sv-${g.weCanServe}`">
          <div class="ep-gap-top">
            <h3>{{ g.gap }}</h3>
            <span class="ep-serve" :class="`sv-${g.weCanServe}`">
              {{ serveLabel[g.weCanServe] }}
            </span>
          </div>

      ...
      <article class="wx-detail" aria-label="Region detail">
        ...
        <div class="wx-block">
          ...
          <svg class="wx-area" viewBox="0 0 320 80" preserveAspectRatio="none" role="img" aria-label="forecast">
            <defs>
              <linearGradient id="wxg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#ffa028" stop-opacity="0.3" /><stop offset="100%" stop-color="#ffa028" stop-opacity="0" /></linearGradient>
            </defs>
            <polygon :points="areaPoints(hiSeries, 320, 80)" fill="url(#wxg)" />
            <polyline :points="sparkPoints(hiSeries, 320, 80)" stroke="#ffa028" fill="none" stroke-width="1.8" />
          </svg>
        </div>
...
const CONDS: Record<Condition, { glyph: string; label: string; color: string }> = {
  ...
  heat: { glyph: '🌡', label: 'Heat', color: '#f85149' },
};
...
<style scoped>
.wx { height: 100%; min-height: 0; display: grid; grid-template-rows: auto auto 1fr; gap: 0.6rem; padding: 0.85rem 1rem 1rem; background: #05070a; color: rgba(255, 255, 255, 0.9); }
.wx-toolbar { display: flex; align-items: center; gap: 1rem; }
.wx-title { display: flex; align-items: baseline; gap: 0.5rem; } .wx-title h1 { margin: 0; font-size: 1rem; letter-spacing: 0.06em; color: #ffa028; font-weight: 700; font-family: 'Roboto Mono', ui-monospace, monospace; text-transform: uppercase; }
...
.wx-tile { flex: 0 0 auto; width: 150px; text-align: left; border: 1px solid #21262d; border-radius: 10px; background: #010409; color: inherit; padding: 0.55rem 0.7rem; cursor: pointer; display: grid; gap: 0.2rem; } .wx-tile:hover { border-color: rgba(255, 255, 255, 0.2); } .wx-tile.on { border-color: #ffa028; }
...
.wx-chg { font-size: 0.72rem; font-weight: 600; margin-left: 0.4rem; font-variant-numeric: tabular-nums; } .wx-chg.up { color: #f85149; } .wx-chg.down { color: #4aa3ff; } .wx-chg.lg { font-size: 0.78rem; }
...
.wx-detail { min-height: 0; overflow-y: auto; border: 1px solid #21262d; border-radius: 12px; padding: 1rem 1.1rem; }
...
.wx-kv { display: flex; align-items: baseline; justify-content: space-between; gap: 0.4rem; font-size: 0.8rem; border-bottom: 1px solid #161b22; padding: 0.3rem 0; } .wx-kv span { color: rgba(255, 255, 255, 0.45); } .wx-kv b { font-variant-numeric: tabular-nums; } .wx-kv b.warn { color: #f0883e; }
.wx-block { margin-top: 0.9rem; border-top: 1px solid #21262d; padding-top: 0.8rem; }
...
.wx-day { display: grid; justify-items: center; gap: 0.2rem; padding: 0.4rem 0.15rem; border: 1px solid #161b22; border-radius: 8px; }
...
.wx-alerts { min-height: 0; overflow-y: auto; border: 1px solid #21262d; border-radius: 12px; padding: 0.5rem; }
...
.wx-alert { text-align: left; border: 1px solid #21262d; border-radius: 8px; background: #010409; color: inherit; padding: 0.5rem 0.6rem; cursor: pointer; display: grid; gap: 0.25rem; } .wx-alert:hover { border-color: rgba(255, 255, 255, 0.2); } .wx-alert.on { box-shadow: 0 0 0 1px #ffa028; }
.wx-alert-top { display: flex; align-items: center; gap: 0.4rem; flex-wrap: wrap; }
.wx-sev { font-size: 0.54rem; text-transform: uppercase; letter-spacing: 0.04em; font-weight: 800; border-radius: 4px; padding: 0.05rem 0.35rem; } .wx-sev.advisory { color: #4aa3ff; background: rgba(74, 163, 255, 0.16); } .wx-sev.watch { color: #f0883e; background: rgba(240, 136, 62, 0.16); } .wx-sev.warning { color: #f85149; background: rgba(248, 81, 73, 0.18); }
.wx-alert-type { font-size: 0.76rem; font-weight: 600; } .wx-res { font-size: 0.56rem; text-transform: uppercase; color: #3fb950; border: 1px solid rgba(63, 185, 80, 0.4); border-radius: 4px; padding: 0.02rem 0.3rem; } .wx-alert-region { margin-left: auto; font-size: 0.68rem; color: rgba(255, 255, 255, 0.45); }

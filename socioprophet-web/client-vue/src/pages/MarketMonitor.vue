<template>
  <section class="mk" aria-label="Market monitor">
    ...
    <header class="mk-toolbar">
      ...
      <div class="mk-asof">as of {{ asOfLabel }}</div>
    </header>
...
<style scoped>
.mk { height: 100%; min-height: 0; display: grid; grid-template-rows: auto auto 1fr; gap: 0.75rem; padding: 1rem 1.25rem 1.25rem; background: #0d1117; color: rgba(255, 255, 255, 0.92); }
.mk-toolbar { display: flex; align-items: center; justify-content: space-between; }
.mk-title { display: flex; align-items: baseline; gap: 0.6rem; } .mk-title h1 { margin: 0; font-size: 1.3rem; }
.mk-pill { font-size: 0.6rem; text-transform: uppercase; letter-spacing: 0.08em; color: #e3b341; background: rgba(227, 179, 65, 0.14); border-radius: 5px; padding: 0.1rem 0.35rem; }
.mk-asof { font-size: 0.74rem; color: rgba(255, 255, 255, 0.45); }

.mk-tiles { display: flex; gap: 0.6rem; overflow-x: auto; padding-bottom: 0.15rem; }
.mk-tile { flex: 0 0 auto; width: 168px; text-align: left; border: 1px solid #21262d; border-radius: 10px; background: #010409; color: inherit; padding: 0.55rem 0.65rem 0.4rem; cursor: pointer; display: grid; gap: 0.25rem; } .mk-tile:hover { border-color: rgba(255, 255, 255, 0.2); } .mk-tile.on { border-color: #58a6ff; }
...
.mk-fbtn { border: none; background: transparent; color: rgba(255, 255, 255, 0.55); border-radius: 6px; padding: 0.15rem 0.45rem; font-size: 0.68rem; text-transform: capitalize; cursor: pointer; } .mk-fbtn.on { background: rgba(88, 166, 255, 0.18); color: #58a6ff; }
.mk-rows { min-height: 0; overflow-y: auto; }
.mk-row { width: 100%; display: grid; grid-template-columns: 1fr 90px 5.5rem 4.5rem; align-items: center; gap: 0.6rem; border: none; border-bottom: 1px solid #161b22; background: transparent; color: inherit; padding: 0.55rem 0.85rem; cursor: pointer; text-align: left; } .mk-row:hover { background: rgba(255, 255, 255, 0.03); } .mk-row.on { background: rgba(88, 166, 255, 0.1); box-shadow: inset 3px 0 0 #58a6ff; }

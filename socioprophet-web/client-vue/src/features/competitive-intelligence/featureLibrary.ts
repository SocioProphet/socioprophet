...
export const featureLibrary: FeatureType[] = [
  ...
  },
  {
    ...
    readiness: 'partial',
    gapNote: 'CORRECTED after auditing the repo: the models and router are real, but BearBrowser ships NO AI sidebar or assistant of any kind — a grep for sidebar/assistant/llm across its actors returns nothing. The capability exists; the delivery surface does not.',
    stance: 'opening',
    ourAngle: 'Owning the browser is a real structural advantage over Grammarly renting it — but it is currently unexercised. Fielding an inline assist means building the AI surface in the browser chrome first; that is the actual work item, not a given.',
  },
  {
  ...
  // ------------------------------ DISTRIBUTION ------------------------------
  {
    ...
    modality: 'none',
    owners: [{ kind: 'estate', name: 'BearBrowser (Firefox-150/LibreWolf fork)', path: '~/dev/BearBrowser' }],
    readiness: 'partial',
    gapNote: 'Verified: real cross-platform binaries ship (v150.0.4 + nightlies) with native actors for blocking, honeypot, nav, vault and capture. But no AI surface, no extension/keyboard/Action-Button surfaces, and builds are unsigned by explicit choice.',
    stance: 'opening',
    ourAngle: 'We OWN the browser rather than renting it, and it already carries native in-page actors — so the hard part (a shipping engine fork with content-script reach) exists. The unexercised part is putting a capability surface on top of it.',
  },
  {
    id: 'in-context-bot',

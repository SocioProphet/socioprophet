...
marker = '<aside class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">'
start = text.find(marker)
if start == -1:
    raise SystemExit("FAILED: could not find Organizations hero aside start")
end = text.find("</aside>", start)
if end == -1:
    raise SystemExit("FAILED: could not find Organizations hero aside end")
end += len("</aside>")
text = text[:start] + aside_html + text[end:]
